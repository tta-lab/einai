...
// Build creates a fantasy.Provider and resolved model ID from a model string.
// Model format: "provider/model-id" or bare model ID (defaults to anthropic).
// Supports: "minimax/" prefix (→ anthropic-compat via MINIMAX_API_URL/MINIMAX_API_KEY)
// and bare model IDs (→ anthropic via ANTHROPIC_API_KEY).
func Build(model string) (fantasy.Provider, string, error) {
	switch {
